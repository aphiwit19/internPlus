...
const SupervisorProfilePage: React.FC<SupervisorProfilePageProps> = ({ user, lang }) => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 px-2">
        <div className="animate-in fade-in slide-in-from-left-4">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none">
            {lang === 'TH' ? 'โปรไฟล์ผู้ดูแล' : 'Supervisor Profile'}
          </h1>
          <p className="text-slate-400 text-sm font-medium mt-3">
            {lang === 'TH' ? 'จัดการข้อมูลบัญชีและภาพรวมของทีม' : 'Manage your account details and team overview.'}
          </p>
        </div>
          ...
          <div className="col-span-12 lg:col-span-8 space-y-8">
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
              <h3 className="text-lg font-black text-slate-900 tracking-tight">
                {lang === 'TH' ? 'ภาพรวมผู้ดูแล' : 'Supervisor Overview'}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                <StatTile
                  label={lang === 'TH' ? 'นักศึกษาที่ดูแล' : 'Assigned Interns'}
                  value={String(internCount)}
                />
                <StatTile label={lang === 'TH' ? 'บทบาท' : 'Role'} value={lang === 'TH' ? 'พี่เลี้ยง' : 'Mentor'} />
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ActionTile
                  title={lang === 'TH' ? 'แก้ไขข้อมูลส่วนตัว' : 'Edit Personal Info'}
                  subtitle={lang === 'TH' ? 'อัปเดตชื่อ เบอร์โทร และข้อมูลการติดต่อ' : 'Update name, phone, and contact info.'}
                  onClick={() => setIsEditing(true)}
                />
              </div>
            </div>
        ...
        <div className="w-full max-w-2xl bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label={t.lineId} value={form.lineId} onChange={(v) => onChange({ ...form, lineId: v })} />
            </div>
