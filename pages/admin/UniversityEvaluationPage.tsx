...
interface AdminUniversityEvaluationPageProps {
  lang: Language;
}

const AdminUniversityEvaluationPage: React.FC<AdminUniversityEvaluationPageProps> = ({ lang }) => {
  const t = useMemo(
    () =>
      ({
        EN: {
          title: 'University Evaluation',
          subtitle: 'Review links and documents submitted by interns.',
        ...
        TH: {
          ...
          subtitle: 'ดูข้อมูลลิงก์และเอกสารที่นักศึกษาส่งมา',
  ...
  useEffect(() => {
    setLoadError(null);
    const q = query(collection(firestoreDb, 'universityEvaluations'));
    return onSnapshot(
      q,
      (snap) => {
        const arr = snap.docs
...
export default AdminUniversityEvaluationPage;
